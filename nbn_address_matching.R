...
setwd("C:/Users/aszwec/Documents/clients/NBN - Business Segmentation/r/")

## Load Libraries
library(stringdist)

## Load some test data
nbn_loc <- read.csv("nbn_location_id.csv", stringsAsFactors = FALSE)
data_provider <- read.csv("one_to_many_address_matching.csv", stringsAsFactors = FALSE)

## Naive attempt at address matching
## Provides a base line for improvement
merge(data_provider, nbn_loc, by="Address", all.x = TRUE )
...
for (m in seq(1, nrow(data_provider))) {
  print( paste( "Matching", nbn_loc$Address , "and", data_provider[m,]$Address, "result is?", amatch(nbn_loc$Address, data_provider[m,]$Address ,maxDist=maxDist) ) )
...
source1.devices<-read.csv('nbn_location_id.csv')
source2.devices<-read.csv('one_to_many_address_matching.csv')
# To make sure we are dealing with charts
source1.devices$name<-as.character(source1.devices$Address)
source2.devices$name<-as.character(source2.devices$Address)
